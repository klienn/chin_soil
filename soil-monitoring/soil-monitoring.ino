...
bool smsSentSoilMoisture = false;
...
void sendSensor() {
  ...
  if (soilMoisture1 < PUMP_THRESHOLD_DRY || soilMoisture2 < PUMP_THRESHOLD_DRY) {
    if (!smsSentSoilMoisture) {
      sendSMS("Either of soil is dry.", 1);
      relayState = true;
    }
  } else if (soilMoisture1 >= PUMP_THRESHOLD_WET || soilMoisture1 >= PUMP_THRESHOLD_WET) {
    smsSentSoilMoisture = false;
    relayState = false;
  }
